...
- 소켓의 프로토콜
    ...
    - 소켓 타입
        ...
        | `TCP` | `UDP` |
        | 중간에 데이터가 소멸되지 x | 데이터 손실 및 파손의 우려 o |
        | 전송 순서대로 데이터 수신 | 전송순서에 상관없이 빠른 속도의 전송 지향 |
        | 데이터의 경계가 존재 x | 데이터의 경계가 존재 o |
        | 소켓 대 소켓의 연결 : 반드시 1 대 1 구조 | 한번에 전송가능한 데이터 크기가 제한 |
    ...
    - 네트워크 바이트 순서와 인터넷 주소 변환
        - 바이트 순서 & 네트워크 바이트 순서
            - CPU가 데이터를 메모리에 저장하는 방식
                ...
                | :---: | :---: |
                | `상위` 바이트의 값을 `작은` 번지수에 저장 | `상위` 바이트의 값을 `큰` 번지수에 저장 |
